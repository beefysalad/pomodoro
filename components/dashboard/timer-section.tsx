...
export function TimerSection() {
  ...
  return (
    <motion.div 
      className="border-violet/20 from-violet/[0.08] flex flex-col items-center gap-6 rounded-2xl border bg-gradient-to-br to-indigo-500/[0.04] p-8 relative overflow-hidden"
    ...
    >
      ...
      <motion.div 
        className="absolute -top-20 -right-20 w-40 h-40 rounded-full bg-violet/10 blur-3xl"
      ...
      <div className="flex gap-2 relative z-10">
        {TIMER_MODES.map((mode) => (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className={`rounded-lg px-4 py-2 text-[11px] font-[700] tracking-[0.04em] transition-all flex items-center gap-1.5 ${
              activeMode === mode.id
                ? 'border-violet-mid/40 bg-violet/20 text-violet-mid border shadow-[0_0_12px_var(--color-violet-glow)]'
                : 'border-border text-muted-foreground border hover:border-violet/40 hover:text-foreground'
            }`}
          >
            {mode.icon}
            {mode.label}
      ...
      <motion.p 
        ...
        animate={{ opacity: 1, y: 0 }}
        exit={{ opacity: 0, y: -10 }}
        className="text-muted-foreground/60 text-[10px] font-[500] tracking-[0.05em] uppercase relative z-10"
      >
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="font-mono text-[64px] md:text-[72px] font-[900] leading-none text-foreground tracking-[-0.06em] relative z-10"
        >
      ...
      >
        <motion.button 
          ...
          whileHover={{ 
            shadow: '0_0_30px_var(--color-violet-glow)',
            y: -2
          }}
          whileTap={{ scale: 0.98 }}
        >
